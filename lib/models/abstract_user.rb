...
class User
  ...
  def self.authenticate(params)
    if params.has_key?(:email)
      current_user = get(:email => params[:email])
    else
      current_user = get(:login => params[:login])
    end
    return nil if current_user.nil?
    return current_user if User.encrypt(params[:password], current_user.salt) == current_user.hashed_password
    nil
  end
